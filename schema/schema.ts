        ...
        UserDetails: {
            ...
            resolve: async (parent, args,{req}):Promise<any> => {
              await isAuthenticateUser(req);                
                ...
                const result = await session.run(`MATCH (u:User {_id:'${args._id}'}) return u`);
    ...
    fields: {
        ...
        registerUser: {
            ...
            resolve: async (parent, args, { res }) => {
                const driver = dbConnection()
                const session = driver.session({ database: "neo4j" });
                const UniqueResult = await session.run(`MATCH (u:User {email :'${args.email}'} ) return u limit 1`)
                if (UniqueResult.records.length != 0) {
                    if (args.email === UniqueResult.records[0].get('u').properties.email) {
                        throw new Error("this email is already used")
                    }
                }
                const unique_id = genUniqueId();
                const result = await session.run(`CREATE (u:User {_id : '${unique_id}', name:'${args.name}',email:'${args.email}',password:'${args.password}'} ) return u`);
        ...
        userUpdate: {
            ...
            resolve: async (parent, args) => {
                ...
                const driver = dbConnection()
                const session = driver.session({ database: "neo4j" });
                const result = await session.run(`MATCH (u:User {_id : '${args._id}'}) SET u.name= '${args.name}', u.email= '${args.email}', u.password= '${args.password}' return u`)
        ...
        loginUser: {
            ...
            resolve: async (parent, args, { res }) => {
                ...
                const driver = dbConnection()
                const session = driver.session({ database: "neo4j" });
                const MatchResult = await session.run(`MATCH (u:User {email :'${args.email}'} ) return u limit 1`)
                if (MatchResult.records.length === 0) {

                    throw new Error("please enter valid email")
                }
                else {
                    if (args.password != MatchResult.records[0].get('u').properties.password) {
